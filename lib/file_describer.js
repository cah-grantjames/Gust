function FileDescriber() {
    ...
    this.findAndDescribeJavaFiles = function(dir) {
        ...
        for (var i = 0; i < filePaths.length; i++) {
            ...
            classes.push({name : fileDescription.className, package : fileDescription.package});
...
//            }
    ...
    this.createFileDescription = function(filePath) {
        ...
        fileDescription.isActivity = this.isActivity(fileDescription.className);
    ...
    this.isActivity = function(className) {
        return this.endsWith(className, "Activity");
    };
        ...
        for(var i = 0; i < lines.length; i++) {
            ...
            if(!inComment && line.trim().substr(0, 2) != "//") {
                for(var j = 0; j < allClasses.length; j++) {
                    var className = allClasses[j].name;
                    var classPackage = allClasses[j].package;
                    if(line.indexOf(className) != -1) {
                        associatedClasses.push(classPackage + "." + className);
                    }
                }
            }
        }
