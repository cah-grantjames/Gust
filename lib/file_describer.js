function FileDescriber() {
        ...
        for (var i=0; i<fileDescriptions.length; i++) {
            ...
            if(fileDescription.isDao) {
                ...
                if(daoInfo.model.dto && daoInfo.model.domain){
                    fileDescriptions = this.associateDomainAndDto(fileDescriptions, fileDescription.className, daoInfo.model.domain, daoInfo.model.dto);
                }
            }
        }
    ...
    this.associateDomainAndDto = function(fileDescriptions, dao, domain, dto) {
        for (var i = 0; i < fileDescriptions.length; i++) {
            fileDescriptions[i].associations = fileDescriptions[i].associations || {hasAssociations: false, dao:[], dto:[], domain:[]};

            if(fileDescriptions[i].className == domain) {
                fileDescriptions[i].associations.hasAssociations = true;
                fileDescriptions[i].associations.dao.push(dao);
                fileDescriptions[i].associations.dto.push(dto);
            }
            if(fileDescriptions[i].className == dto) {
                fileDescriptions[i].associations.hasAssociations = true;
                fileDescriptions[i].associations.dao.push(dao);
                fileDescriptions[i].associations.domain.push(domain);
            }
            fileDescriptions[i].associations.dao = fileDescriptions[i].associations.dao.getUnique()
            fileDescriptions[i].associations.dto = fileDescriptions[i].associations.dto.getUnique()
            fileDescriptions[i].associations.domain = fileDescriptions[i].associations.domain.getUnique()
