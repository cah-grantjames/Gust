function FileDescriber() {
    ...
    this.findAndDescribeJavaFiles = function(dir) {
        ...
        for (var i = 0; i < fileDescriptions.length; i++) {
            this.findInternalBroadcastReceiverInfo(fileDescriptions[i]);
        }

        return fileDescriptions;
    };

    this.findInternalBroadcastReceiverInfo = function(fileDescription) {
        if(!fileDescription.broadcastReceiverInfo) {
            fileDescription.broadcastReceiverInfo = {declared: []};
        }
        ...
        for(var i = 0; i < lines.length; i++) {
            ...
            if(finds.type.index != -1 && finds.instantiation.index != -1){
                ...
                fileDescription.broadcastReceiverInfo.declared.push(brname);
            }
        }
    };
