function HtmlCreator() {
    ...
    this.createHtmlDocs = function(outputDir, outputFileName, fileDescriptions) {
        ...
        this.fs.writeFileSync(outputDir + "/" + outputFileName, content);
    };
            ...
            if(tokens[j].type == "property") {
                ...
                for(var x = 0; x < propertyList.length; x++) {
                    ...
                    if(tokens[j].tokenProperty == "*"){
                        text += listTemplate.replace(tokens[j].templateToken, p);
                    } else {
                        text += listTemplate.replace(tokens[j].templateToken, this.readProperty(p, tokens[j].tokenProperty));
                    }
                }
